...
class GdgListFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.showNeedLocation.observe(
            viewLifecycleOwner
        ...
        viewModel.regionList.observe(viewLifecycleOwner, object : Observer<List<String>> {
            override fun onChanged(data: List<String>?) {
                ...
                with(binding) {
                    val inflator = LayoutInflater.from(regionList.context)

                    val children = data.map { regionName ->
                        val chip = inflator.inflate(R.layout.region, regionList, false) as Chip
                        chip.text = regionName
                        chip.tag = regionName
                        chip.setOnCheckedChangeListener { button, isChecked ->
                            this@GdgListFragment.viewModel.onFilterChanged(button.tag as String, isChecked)
                        }
                        chip
                    }
